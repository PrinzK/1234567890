...
def line_follower(run_event,in_q_dist, in_q_stop):
    ...
    while run_event.is_set():
		...
		#print 'area -1'
            ...
            print time.time(), 'Stopping motors'
            pi2go.stop()
        #lock.release()
        # LED-Control
        if emergency_stop:
...
def distance(run_event, out_q):
    ...
    while run_event.is_set():
        ...
        if curr_time - 0.1 > start:
			...
			out_q.put(dist)
	if dist < 10:
		print time.time(), 'broadcast function about to be called'
		broadcast('STOP')
		stop_sent = True
...
def broadcast(message):
	...
	print time.time(), 'broadcastfunction finished'
